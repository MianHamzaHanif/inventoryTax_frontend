...
const Department = () => {
  ...
  const { itemHeads, totalPages: reduxTotalPages } = useSelector(
  ...
  useEffect(() => {
    // Fetch item heads and update totalPages
    dispatch(fetchDepartment({ page: pagination.page, limit: pagination.limit }))
      .unwrap()
      .then((response) => {
        // Here, we set the totalPages from the response parameters
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      ...
      dispatch(
        updateItemHead({ id: itemHeadDetails._id, name: itemHeadDetails.name })
      )
        .unwrap()
        .then(() => {
      ...
      dispatch(addItemHead({ name: itemHeadDetails.name }))
        ...
        .then(() => {
          ...
          // Re-fetch the item heads to update the table
          dispatch(
            fetchDepartment({ page: pagination.page, limit: pagination.limit })
          );
          resetForm();
        })
        .catch((error) => {
  ...
  const handleDelete = (id) => {
    dispatch(deleteItemHead(id))
      ...
      .catch((error) =>
        toast.error(`Failed to delete item head: ${error.message}`)
      );
  };
  ...
  const resetForm = () => {
    ...
    if (isEditing) {
      // Use pagination.page instead of currentPage
      dispatch(
        fetchDepartment({ page: pagination.page, limit: pagination.limit })
      );
    }
  ...
  return (
    <section>
      <div className="container">
        <div className="nine">
          <h1 className="mt-4">
            Department<span>HiTech Solution</span>
          </h1>
          {showForm && (
            <form onSubmit={handleSubmit}>
              ...
              required
            /> */}
              <button type="submit" className="new_btn2 mt-2 px-4 p-2">
                {isEditing ? "Update" : "Add"} Item Head
              </button>
            </form>
          )}
          {!showForm && (
              ...
              <tbody>
                {itemHeads?.map((itemHead, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <MdDeleteOutline
                        onClick={() => handleDelete(itemHead._id)}
                      />
                    </td>
                  </tr>
                ))}
